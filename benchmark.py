...
def run_benchmark(f0, D, w_inc, label):
    ...
    dx = 1e-8
    By_plus = evaluate_SC_2D(x_vec + dx, z_vec, x_m, z_m, nx, nz, dl, By_inc, dBy_dn_inc, k)
    By_minus = evaluate_SC_2D(x_vec - dx, z_vec, x_m, z_m, nx, nz, dl, By_inc, dBy_dn_inc, k)
...
# High f-number setup (f_eff = 100um, w_inc = 15um) -> expect clean paraxial waist
f0_parax = 50e-6
D_parax = 20e-6
w_inc_parax = 15e-6
...
popt, _ = curve_fit(gaussian_1d, z_p, Ez_p, p0=[np.max(Ez_p), 0.0, 2e-6])
...
print(f"Waist Agreement Error:      {abs(w_numerical - w_theoretical)/w_theoretical * 100:.2f}%")
...
print("\nSaved benchmark visualization to 'benchmark_results.png'.")